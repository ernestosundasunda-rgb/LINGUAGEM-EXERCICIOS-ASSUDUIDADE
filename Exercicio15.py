...
dados_vendas = [
    {"produto": "cOMPUTADOR", "categoria": "Eletrônicos", "quantidade": 10, "preco_unitario": 1200},
...
def gerar_relatorio(vendas, filtro_fn=None, transformacao_fn=None):
    ...
    vendas_transformadas = (
        list(map(transformacao_fn, vendas_filtradas))
        if transformacao_fn
        else vendas_filtradas
    )
...
# Relatório de eletrônicos
print("\n--- Relatório de Eletrônicos (Total de Vendas) ---")
relatorio_eletronicos = gerar_relatorio(
    dados_vendas,
    filtro_fn=filtro_eletronicos,
    transformacao_fn=transformacao_total_vendas
)
...
print("\n--- Relatório de Acessórios (Nome e Quantidade) ---")

filtro_acessorios = lambda item: item["categoria"] == "Acessórios"

transformacao_nome_quantidade = lambda item: {
    "produto": item["produto"],
    "quantidade": item["quantidade"]
}

relatorio_acessorios = gerar_relatorio(
    dados_vendas,
    filtro_fn=filtro_acessorios,
    transformacao_fn=transformacao_nome_quantidade
)

for item in relatorio_acessorios:
    print(item)